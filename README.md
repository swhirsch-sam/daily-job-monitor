...
Edit .github/workflows/daily_scan.yml and change the cron expression. The default is 8:00 AM UTC daily. Use crontab.guru to build your preferred schedule.
...
6. Results are compared against seen_jobs.json to detect new vs. still-open roles.
7. An email is sent if there is at least one new role.
8. seen_jobs.json is updated and committed back to the repo for next-day deduplication.

## Email Format

Subject: Daily Job Scan - [Date] - [X] New Role(s) Found

Body includes new roles with company, title, location, link and date posted; still-open roles from prior scans; per-site match counts; and any scan errors.
...
All filter keywords and site URLs are defined at the top of job_monitor.py in the INCLUDE_KEYWORDS, EXCLUDE_KEYWORDS, and SITES lists. Edit them without touching the scraping logic.
